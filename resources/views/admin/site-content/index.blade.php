...
<div class="content-manager">
    <div class="content-hero">
        ...
        <div class="content-hero-stat"><strong>{{ $contents->count() }}</strong><span>éléments gérés</span></div>
    </div>

    @if(session('success'))<div class="alert alert-success content-alert">{{ session('success') }}</div>@endif
    @if($errors->any())<div class="alert alert-danger content-alert">{{ $errors->first() }}</div>@endif
    ...
    <div class="content-toolbar">
        ...
        <div class="content-filters" role="tablist" aria-label="Filtrer les sections">
            <button type="button" class="content-filter is-active" data-filter="all">Tout <b>{{ $contents->count() }}</b></button>
            @foreach($sectionMeta as $section => $meta)
                @if($groupedContents->has($section))<button type="button" class="content-filter" data-filter="{{ $section }}">{{ $meta['title'] }} <b>{{ $groupedContents[$section]->count() }}</b></button>@endif
            @endforeach
        </div>
    </div>
    ...
    <section class="content-add card">
        ...
        <form method="POST" action="{{ route('admin.site-content.store') }}" class="content-form">
            @csrf
            <div class="field"><label for="new-section">Section</label><select id="new-section" name="section" required>@foreach($sectionMeta as $section => $meta)<option value="{{ $section }}">{{ $meta['title'] }}</option>@endforeach</select></div>
    ...
    <div class="content-sections">
        @foreach($sectionMeta as $section => $meta)
            @if($groupedContents->has($section))
                <section class="content-section" data-content-section="{{ $section }}">
                    <div class="content-section-title"><div class="content-section-icon">{{ $meta['icon'] }}</div><div><h3>{{ $meta['title'] }}</h3><p>{{ $meta['description'] }}</p></div><span class="content-count">{{ $groupedContents[$section]->count() }} éléments</span></div>
                    <div class="content-items">
                        @foreach($groupedContents[$section] as $content)
                            <article class="content-item" data-content-search="{{ strtolower($content->section.' '.$content->key.' '.$content->label_fr.' '.$content->label_en.' '.$content->value_fr.' '.$content->value_en) }}">
                                <form method="POST" action="{{ route('admin.site-content.update', $content) }}" class="content-edit-form">
                                    @csrf @method('PUT')
                                    <div class="content-item-top"><div class="content-item-number">{{ str_pad($loop->iteration, 2, '0', STR_PAD_LEFT) }}</div><div class="content-item-summary"><strong>{{ $content->label_fr }}</strong><small>{{ $content->key }}</small></div><label class="visibility-toggle compact"><input type="checkbox" name="is_published" value="1" {{ $content->is_published ? 'checked' : '' }}><span></span><b>{{ $content->is_published ? 'Publié' : 'Masqué' }}</b></label></div>
                                    <div class="content-edit-grid">
                                        <div class="field"><label>Titre / année FR</label><input name="label_fr" value="{{ $content->label_fr }}" required></div>
                                        <div class="field"><label>Titre / année EN</label><input name="label_en" value="{{ $content->label_en }}"></div>
                                        <div class="field field-wide"><label>Valeur / description FR</label><textarea name="value_fr" rows="3">{{ $content->value_fr }}</textarea></div>
                                        <div class="field field-wide"><label>Valeur / description EN</label><textarea name="value_en" rows="3">{{ $content->value_en }}</textarea></div>
                                        <div class="field"><label>Icône</label><input name="icon" value="{{ $content->icon }}" placeholder="📊"></div>
                                        <div class="field"><label>Suffixe</label><input name="suffix" value="{{ $content->suffix }}" placeholder="%, km..."></div>
                                        <div class="field"><label>Ordre</label><input type="number" name="sort_order" value="{{ $content->sort_order }}" min="0"></div>
                                        <input type="hidden" name="section" value="{{ $content->section }}"><input type="hidden" name="key" value="{{ $content->key }}">
                                    </div>
                                    <div class="content-item-actions"><span class="content-item-id">ID : {{ $content->key }}</span><button class="btn btn-primary" type="submit">Enregistrer les modifications</button></div>
                                </form>
                                <form method="POST" action="{{ route('admin.site-content.destroy', $content) }}" class="delete-form" onsubmit="return confirm('Supprimer cet élément ?');">@csrf @method('DELETE')<button type="submit" title="Supprimer cet élément">Supprimer</button></form>
                            </article>
                        @endforeach
                    </div>
                </section>
