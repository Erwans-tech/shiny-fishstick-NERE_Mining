...
<section class="sa-animated-section">
    ...
    <div class="sa-section-heading sa-reveal" style="margin-bottom:32px;">
        <h2 style="text-align:left;">{{ __('site.nav_company', [], $loc) }}</h2>
        <div class="sa-divider" style="margin: 0;"></div>
    </div>

    <p class="lead sa-reveal sa-delay-1">{{ __('site.company_identity_lead', [], $loc) }}</p>

    <div class="grid-3" style="margin-top:32px;">
        <a href="{{ $en ? route('english.company.ceo') : route('company.ceo') }}" class="sa-program-card sa-reveal sa-delay-1" style="display:block; text-decoration:none;">
            ...
            <span class="sa-btn-animated" style="margin-top:16px; display:inline-flex;">
                <span>{{ __('site.discover', [], $loc) }}</span>
                <span class="sa-btn-arrow">→</span>
            </span>
        </a>
        <a href="{{ $en ? route('english.company.identity') : route('company.identity') }}" class="sa-program-card sa-reveal sa-delay-2" style="display:block; text-decoration:none;">
        ...
        <a href="{{ $en ? route('english.company.history') : route('company.history') }}" class="sa-program-card sa-reveal sa-delay-3" style="display:block; text-decoration:none;">
        ...
        <a href="{{ $en ? route('english.company.values') : route('company.values') }}" class="sa-program-card sa-reveal sa-delay-1" style="display:block; text-decoration:none;">
        ...
        <a href="{{ $en ? route('english.company.governance') : route('company.governance') }}" class="sa-program-card sa-reveal sa-delay-2" style="display:block; text-decoration:none;">
...
<section class="sa-sand-animated" style="padding:70px 5vw; position:relative;">
    ...
    <div style="max-width:1180px; margin:0 auto; position:relative; z-index:1;">
        <div class="sa-section-heading sa-reveal">
            <h2>{{ $en ? 'Our Mission & Vision' : 'Notre Mission & Vision' }}</h2>
...
<section class="sa-animated-section" style="padding:70px 5vw;">
    <div style="max-width:1180px; margin:0 auto;">
        <div class="sa-section-heading sa-reveal">
            <h2>{{ $en ? 'Our Core Values' : 'Nos Valeurs Fondamentales' }}</h2>
            <div class="sa-divider"></div>
            <p style="color:var(--muted); font-size:15px; line-height:1.8; margin:0;">
                {{ $en ? 'These values guide every decision and action at Néré Mining' : 'Ces valeurs guident chaque décision et action chez Néré Mining' }}
            </p>
        </div>
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:24px; margin-top:48px;">
            ...
            <div class="sa-partner-card sa-reveal sa-delay-{{ $k+1 }}" style="text-align:center;">
                <div class="company-value-icon">{{ $val['icon'] }}</div>
                <h4 style="color:var(--green); margin:0 0 8px 0; font-size:16px; font-weight:600;">{{ $val['title'] }}</h4>
                <p style="color:var(--muted); font-size:13px; margin:0; line-height:1.6;">{{ $val['desc'] }}</p>
            </div>
            @endforeach
        </div>
    </div>
