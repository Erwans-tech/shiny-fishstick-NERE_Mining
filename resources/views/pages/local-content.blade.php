    ...
    <div style="max-width:1180px; margin:0 auto;">
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(160px, 1fr)); gap:24px; margin-bottom:40px;">
            <div style="background:#fff; padding:24px; border-radius:8px; text-align:center; border:1px solid var(--line);">
                ...
                <div style="font-size:13px; color:var(--muted); text-transform:uppercase; letter-spacing:.06em;">{{ $en ? 'Local & regional employment' : 'Emploi local et régional' }}</div>
                <div style="font-size:12px; color:var(--muted); margin-top:8px;">{{ $en ? 'of the workforce' : 'de la main-d’œuvre' }}</div>
            </div>
            <div style="background:#fff; padding:24px; border-radius:8px; text-align:center; border:1px solid var(--line);">
                ...
                <div style="font-size:12px; color:var(--muted); margin-top:8px;">{{ $en ? 'CFA' : 'FCFA' }}</div>
            </div>
            <div style="background:#fff; padding:24px; border-radius:8px; text-align:center; border:1px solid var(--line);">
                <div style="font-size:32px; font-weight:700; color:var(--green); margin-bottom:8px;">744 280 000</div>
                <div style="font-size:13px; color:var(--muted); text-transform:uppercase; letter-spacing:.06em;">{{ $en ? 'State-owned company payments' : "Paiements aux Entreprises d'État" }}</div>
                <div style="font-size:12px; color:var(--muted); margin-top:8px;">{{ $en ? 'CFA' : 'FCFA' }}</div>
            </div>
            <div style="background:#fff; padding:24px; border-radius:8px; text-align:center; border:1px solid var(--line);">
                <div style="font-size:32px; font-weight:700; color:var(--green); margin-bottom:8px;">99%</div>
                <div style="font-size:13px; color:var(--muted); text-transform:uppercase; letter-spacing:.06em;">{{ $en ? 'Burkinabe workers' : 'Travailleurs burkinabè' }}</div>
                <div style="font-size:12px; color:var(--muted); margin-top:8px;">{{ $en ? 'of the workforce' : 'de la main-d’œuvre' }}</div>
            </div>
            <div style="background:#fff; padding:24px; border-radius:8px; text-align:center; border:1px solid var(--line);">
                <div style="font-size:32px; font-weight:700; color:var(--green); margin-bottom:8px;">1 500</div>
                <div style="font-size:13px; color:var(--muted); text-transform:uppercase; letter-spacing:.06em;">{{ $en ? 'Subcontracted workers' : 'Travailleurs sous-traitants' }}</div>
                <div style="font-size:12px; color:var(--muted); margin-top:8px;">{{ $en ? 'supporting operations' : 'en appui aux opérations' }}</div>
            </div>
        </div>
    </div>
</section>
