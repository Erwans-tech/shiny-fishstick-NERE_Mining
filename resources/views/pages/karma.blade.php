...
<section id="presentation" class="sa-animated-section" style="padding-top:40px;">
    ...
    <p class="lead sa-reveal sa-delay-1">{{ __('site.karma_pres_lead', [], $loc) }}</p>
    ...
    <div class="grid-2" style="margin-bottom:40px; margin-top:32px;">
        <div>
            <div class="sa-program-card sa-reveal sa-delay-1" style="margin-bottom:20px;">
                <h4 style="color:var(--green); margin-bottom:12px; text-align:left;">{{ __('site.karma_history_h4', [], $loc) }}</h4>
                <p>{{ __('site.karma_history_p', [], $loc) }}</p>
            </div>
            <div class="sa-program-card sa-reveal sa-delay-2" style="margin-bottom:20px;">
                <h4 style="color:var(--green); margin-bottom:12px; text-align:left;">{{ __('site.karma_loc_h4', [], $loc) }}</h4>
                <p>{!! nl2br(e(__('site.karma_loc_p', [], $loc))) !!}</p>
            </div>
            <div class="sa-program-card sa-reveal sa-delay-3">
                <h4 style="color:var(--green); margin-bottom:12px; text-align:left;">{{ __('site.karma_area_h4', [], $loc) }}</h4>
    ...
    <div style="position:relative; z-index:1; max-width:1180px; margin:0 auto;">
        ...
        <div class="grid-3" style="margin-top:40px;">
            ...
            <div class="sa-program-card sa-reveal sa-delay-3 karma-production-card karma-production-card--team" style="padding:0; overflow:hidden;">
                <img class="card-img" style="width:100%; height:200px; object-fit:cover;" src="{{ asset('images/mining/karma-01.jpg') }}" alt="{{ $en ? 'Burkinabe mining team' : 'Équipe minière burkinabè' }}">
                <div style="padding:24px;">
                    <h3 style="color:var(--green); margin-bottom:12px; text-align:left;">{{ __('site.karma_card3_h3', [], $loc) }}</h3>
...
<section id="production-timeline" class="sa-animated-section" style="padding:70px 5vw;">
    ...
    <div class="sa-program-card sa-reveal" style="max-width:800px; margin:0 auto;">
        <h3 style="color:var(--green); margin:0 0 16px 0; font-size:18px; font-weight:600; text-align:left;">{{ $en ? 'Key Milestones' : 'Jalons Clés' }}</h3>
        <ul class="sa-animated-list">
            <li><span class="sa-list-bullet">✓</span><span style="font-size:14px; color:var(--muted);">{{ $en ? '2007: Acquired Goulagou and Rounga properties (487 km²)' : '2007: Acquisition des propriétés Goulagou et Rounga (487 km²)' }}</span></li>
            <li><span class="sa-list-bullet">✓</span><span style="font-size:14px; color:var(--muted);">{{ $en ? '2019: Commenced gold production at 80 Koz annually' : '2019: Démarrage production or à 80 Koz annuels' }}</span></li>
            <li><span class="sa-list-bullet">✓</span><span style="font-size:14px; color:var(--muted);">{{ $en ? '2024: Transition to Burkinabè-majority ownership (Néré Mining)' : '2024: Transition vers majorité actionnaire burkinabè (Néré Mining)' }}</span></li>
            <li><span class="sa-list-bullet">✓</span><span style="font-size:14px; color:var(--muted);">{{ $en ? '2026+: CIL plant commissioning for refractory ore processing' : '2026+: Mise en service usine CIL pour traitement minerai réfractaire' }}</span></li>
        </ul>
    </div>
</section>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:28px; align-items:start; margin-bottom:48px;">
                ...
                <table style="width:100%; border-collapse:collapse; font-size:14px; line-height:1.6;">
                    ...
                    <tbody>
                        ...
                        <tr style="background:var(--sand); font-weight:600; border-top:2px solid var(--gold);">
                            <td style="padding:14px 16px;">{{ $en ? 'Total' : 'Total' }}</td>
            ...
            <div>
                <div class="sa-program-card sa-reveal sa-delay-2" style="margin-bottom:16px;">
                    <h3 style="color:var(--green); margin-bottom:12px; font-size:16px; font-weight:600; text-align:left;">{{ $en ? 'Resource Overview' : 'Vue d\'Ensemble des Ressources' }}</h3>
                ...
                <div class="sa-partner-card sa-reveal sa-delay-3" style="border-left:4px solid var(--green);">
                    <h4 style="color:var(--green); margin:0 0 12px 0; font-size:14px; font-weight:600; text-align:left;">{{ $en ? 'Classification' : 'Classification' }}</h4>
                    <ul class="sa-animated-list">
                        <li><span class="sa-list-bullet" style="font-size:11px;">•</span><span style="font-size:13px; color:var(--muted);">{{ $en ? 'Measured Resources: High confidence deposits' : 'Ressources Mesurées: Gisements haute confiance' }}</span></li>
                        <li><span class="sa-list-bullet" style="font-size:11px;">•</span><span style="font-size:13px; color:var(--muted);">{{ $en ? 'Indicated Resources: Established geological continuity' : 'Ressources Indiquées: Continuité géologique établie' }}</span></li>
                        <li><span class="sa-list-bullet" style="font-size:11px;">•</span><span style="font-size:13px; color:var(--muted);">{{ $en ? 'Inferred Resources: Limited geological evidence' : 'Ressources Inférées: Preuves géologiques limitées' }}</span></li>
                    </ul>
                </div>
            </div>
        </div>
...
<section id="organisation" class="sa-animated-section" style="padding:70px 5vw;">
    ...
    <div class="grid-3">
        ...
        @endphp
        <div class="sa-program-card sa-reveal sa-delay-{{ $loop->iteration % 3 + 1 }}">
            <div class="card-tag">{{ $deptTag !== '' ? $deptTag : __('site.karma_dept'.$loop->iteration.'_tag', [], $loc) }}</div>
            <h3 style="text-align:left;">{{ $deptTitle !== '' ? $deptTitle : __('site.karma_dept'.$loop->iteration.'_h3', [], $loc) }}</h3>
            <p>{{ $deptBody !== '' ? $deptBody : __('site.karma_dept'.$loop->iteration.'_p', [], $loc) }}</p>
        </div>
        @empty
        @foreach(range(1, 9) as $i)
        <div class="sa-program-card sa-reveal sa-delay-{{ $i % 3 + 1 }}">
            <div class="card-tag">{{ __('site.karma_dept'.$i.'_tag', [], $loc) }}</div>
            <h3 style="text-align:left;">{{ __('site.karma_dept'.$i.'_h3', [], $loc) }}</h3>
    ...
    <div style="position:relative; z-index:1; max-width:1180px; margin:0 auto;">
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:24px;">
            ...
            <div class="sa-step-card sa-reveal sa-delay-{{ $i }}" data-step="{{ $i }}">
                <h4 style="color:var(--green); margin-bottom:12px; font-size:18px;">{{ __('site.karma_step'.$i.'_h4', [], $loc) }}</h4>
                <p style="font-size:14px; line-height:1.6; color:var(--muted);">{{ __('site.karma_step'.$i.'_p', [], $loc) }}</p>
            </div>
            @endforeach
        </div>
    </div>
...
<section id="impact" class="sa-animated-section" style="padding:70px 5vw;">
    ...
    <div class="grid-2">
        <div>
            <h3 class="sa-reveal" style="text-align:left; color:var(--green); margin-bottom:24px;">{{ __('site.karma_imp_jobs_h3', [], $loc) }}</h3>
            @foreach(range(1, 3) as $i)
            <div class="sa-achievement-card sa-reveal sa-delay-{{ $i }}" style="margin-bottom:18px; text-align:left;">
                <div class="card-tag">{{ __('site.karma_imp_job'.$i.'_tag', [], $loc) }}</div>
                <p style="margin-top:8px;">{{ __('site.karma_imp_job'.$i.'_p', [], $loc) }}</p>
            </div>
            @endforeach
        </div>
        <div>
            <h3 class="sa-reveal" style="text-align:left; color:var(--green); margin-bottom:24px;">{{ __('site.karma_imp_eco_h3', [], $loc) }}</h3>
            @foreach(range(1, 3) as $i)
            <div class="sa-achievement-card sa-reveal sa-delay-{{ $i }}" style="margin-bottom:18px; text-align:left;">
                <div class="card-tag">{{ __('site.karma_imp_eco'.$i.'_tag', [], $loc) }}</div>
                <p style="margin-top:8px;">{{ __('site.karma_imp_eco'.$i.'_p', [], $loc) }}</p>
            </div>
            @endforeach
        </div>
    </div>
